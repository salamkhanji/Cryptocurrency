...
class App extends Component {
  ...
  componentDidMount() {
    const owner =  "0xD8776f9656A71db511eb7CA448944be5e7823ff8";        // change this line with your current address;
  ...
  getTokens = async event => {
    ...
    const accounts = await web3.getTokens().send({ from: this.state.owner , value: this.state.value});
...
console.log(result)
  ...
  transfer = async event => {
    event.preventDefault();
    const accounts = await web3.transfer();
    this.setState({ message: 'Waiting on transaction success...' });
     await // your code here;
